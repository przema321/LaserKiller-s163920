	...
	while(!done)
	{
		...
		if(redraw && al_is_event_queue_empty(event_queue))
		{
			...
			if(!isGameOver)
			{
				...
				al_draw_textf(font18, al_map_rgb(255, 0, 255), 5, 5, 0, "Zycia: %i Zniszczone obiekty: %i", ship.lives, ship.score);
			}
			else
			{
				al_draw_textf(font18, al_map_rgb(0, 255, 255), WIDTH / 2, HEIGHT / 2, ALLEGRO_ALIGN_CENTRE, "Gra Skonczona. Twoj wynik: %i", ship.score);
			}
